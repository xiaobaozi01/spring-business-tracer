...
主Agent必须传入VALIDATE claim的`batchId/batchToken/workerId`和`lease.heartbeatDueAt`；验证跨过续租时点前调用`spring_state_heartbeat`，每次真实续租换新的operationId。不写文件、不发布、不调用 Subagent；除报告工具和heartbeat外不调用状态工具。

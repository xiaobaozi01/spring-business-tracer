...
重新读取出站与入站源码；应用properties/yml/yaml只能通过`spring_config_resolve`取得脱敏值与来源hash，禁止直接read。确认 source Java 路径由 Code Graph 到达边界点并规范化双方 key。HTTP 要求唯一目标；Kafka 按 cluster/topic/group 核对，不同 group fan-out，同 group 多 listener 形成 COMPETING_ONE_OF targets。动态、配置冲突或协议不一致返回 UNRESOLVED/REJECTED；不得任选。提交前调用`spring_report_context`，通过时在结构化`report.verifiedBoundaryIds`写入精确稳定ID，然后调用`spring_report_submit`；图构建只接受ACCEPTED认证集合。

...
执行完整扫描：Doctor→spring_config_resolve→config auditor提交CONFIG报告→`spring_discovery_claim`按服务领取发现租约→每个entry worker用`spring_discovery_commit`直接checkpoint结构化清单→`spring_discovery_status`确认无缺失→plan（不手工拼entriesJson）→分批trace/验证/发布→coverage/boundary→V2分片拓扑→COMPLETE。中断后只重试非COMPLETE服务；批次定期heartbeat并close。

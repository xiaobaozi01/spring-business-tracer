...
const queryLog = [{ tool: "codegraph_callees", args: { limit: 101 }, purpose: "验证Java调用边", resultCount: 1, truncated: false }];
...
const samples = {
  ...
  run: { schemaVersion: "2.0", runId: "run-schema", phase: "PLANNED", configHash: hash, sourceSnapshot: hash, indexFingerprint: hash, toolkitFingerprint: hash, resolutionContextHash: hash, adapterRegistryFingerprint: hash, mode: "FULL", units: { entry: { id: "entry", service: "order", kind: "HTTP", status: "PENDING", attempts: 0, fingerprintToken: null } }, audits: { coverageHash: null, boundaryHash: null, configHash: null } },
  "entry-inventory": { schemaVersion: "2.0", runId: "run-schema", fingerprints, adapters: [{ name: "SPRING_MVC", enabled: true, count: 1 }], excludedCandidates: [], entries: [{ id: "order:http:GET:/orders", serviceId: "order", adapter: "SPRING_MVC", adapterDefinitionVersion: "2.0.0", contextIds: ["prod-cn"], conditions: [], visibility: "PUBLIC", trigger: "GET /orders", symbolId: "OrderController#get", signature: "get()", file: "OrderController.java", line: 1, codeGraphQuery: { limit: 101 } }] },
  "boundary-link": boundary,
  "trace-result": { schemaVersion: "2.0", runId: "run-schema", entryId: "entry", fingerprints, status: "TRACED", contextIds: ["prod-cn"], configDependencyIds: ["billing.queue"], serviceClosure: ["order"], javaEdges: [{ from: "a", to: "b", tool: "codegraph_callees", query: { limit: 101 }, file: "A.java", line: 1 }], boundaries: [boundary], persistence: [{ symbol: "b", resource: "db:main:table:sales.orders", storeId: "main", resourceKind: "RELATIONAL_TABLE", operation: "READ", evidence }], topologyFacts: [{ type: "DISPATCHES_TO", from: "entry", to: "a", assurance: "VERIFIED", provenance: evidence }], unresolvedFindings: [], queryLog },
  ...
  manifest: { schemaVersion: "2.0", toolkitVersion: "2.0.0", runId: "run-schema", graphHash: hash, fingerprints, contexts: [{ id: "prod-cn", contextHash: hash }], services: [{ id: "order", root: "order" }], entrypoints: [{ id: "entry", status: "PUBLISHED" }], tables: [], boundaries: [], codeGraphTools: ["codegraph_callees"], graph: { nodeCount: 2, edgeCount: 1, formatVersion: 2, topologyRootHash: hash }, tombstones: { entryIds: [], nodeIds: [], edgeIds: [] }, graphDelta: null, audits: { coverageHash: hash, boundaryHash: hash, configHash: hash } },
